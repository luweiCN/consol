...
consol/
├── packages/
│   ├── cli/                 # TS/Bun CLI 入口，命令名 consol
│   ├── core/                # 产品状态、配置、目标解析
│   ├── foundry/             # forge/cast/anvil 适配层
│   ├── i18n/                # en-US 和 zh-CN 文案目录
│   ├── packaging/           # Bun compile 和二进制 smoke
│   ├── protocol/            # JSON/NDJSON envelope 和事件
│   ├── testkit/             # Fake Foundry 工具和行为合同 fixtures
│   └── tui/                 # OpenTUI/Solid 终端界面
├── docs/
...
主产品是 `packages/*` 下的 `consol` TS/Bun CLI 和 OpenTUI/Solid TUI。VS Code 和 NeoVim 集成计划作为同一套 CLI/JSON/NDJSON protocol 上的轻量客户端，而不是新的产品分支。
