...
`consol dev` scans Solidity sources under `src`, `contracts`, `test`, `script`, and root-level demo `.sol` files. The TUI centers on the currently selected contract: press `/` to open a fuzzy contract picker, `b` to build the ABI, `d` to preview/deploy, and arrow keys plus `Enter` or `c` to run read/write functions when the function pane is focused. The Contract workspace follows a terminal cockpit model with a compact context strip, focused runnable ABI list, selected-row details, persistent State Watch, Activity, and bottom keybar. `Tab` / `Shift-Tab` cycles focus between panes and `[` / `]` switches top workspace tabs; mouse input never changes pane focus. Activity scrolls with PageUp/PageDown or mouse wheel only when the Activity pane is already focused and the pointer is over that pane, and workspace shortcuts are ignored briefly after wheel input to avoid terminal/tmux mixed-event tab switches. If a function needs a deployment first, ConSol opens the deploy preview instead of leaving you at a dead `no deployment` state. Argument input is remembered per function during the session, zero-argument read state is shown in the Contract workspace and State panel, and durable panel data comes from the same CLI layer as `consol state`, `consol logs`, `consol tx list`, and `consol activity`.

User profiles live in `~/.config/consol/config.toml`. UI language can be set there with `[ui] language = "zh-CN"` or `"en-US"`; config wins over locale environment variables, while `"system"` falls back to `CONSOL_LANG`, `LANGUAGE`, `LC_ALL`, `LC_MESSAGES`, or `LANG`. `consol dev` also writes session and crash diagnostics to `~/.config/consol/logs/consol-dev.log` so TUI exits can be debugged after the terminal restores.
