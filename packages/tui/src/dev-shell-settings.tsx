...
export const settingsSections = ["language", "stateDisplay"] as const;
...
function SettingsMenuRow(props: {
  ...
  return (
    ...
    >
      ...
      <text flexShrink={0} fg={props.selected ? theme.color.selected : theme.color.text} content={`${props.icon} ${props.title}`} />
      <text flexShrink={0} fg={selectedReadableColor(props.selected, theme.color.border)} content="  " />
      <box
        height={1}
        flexDirection="row"
      >
        <box height={1} onMouseDown={(event: MouseEvent) => runMouseValueAction(event, props.onSelect, props.onValuePrev)}>
          <text flexShrink={0} fg={selectedReadableColor(props.selected, theme.color.border)} content="< " />
        </box>
        <box height={1} onMouseDown={(event: MouseEvent) => runMouseValueAction(event, props.onSelect, props.onValueNext)}>
...
function runMouseValueAction(event: MouseEvent, onSelect: () => void, onValueChange: () => void): void {
