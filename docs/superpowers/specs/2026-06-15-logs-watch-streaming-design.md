...
- 第一行：`type: "watch_started"`，`sequence: 0`，`data` 含 `contract` / `address` / `network`。
  让管道消费者确认订阅已建立、知道监听目标。
- 之后每条事件：`type: "log"`，`sequence` 从 1 起递增，`data` = `DecodedLog`。
...
用例：
1. **ndjson 流**：断言收集到 `[watch_started(seq0), log(seq1)]`，每行可 `JSON.parse` 且过 `CliNdjsonEventSchema`，
