...
The first action inside `dev` is the Functions panel action sheet: when the `Functions` tab is active, `j/k` move the selected ABI function and `Enter` or `c` opens the relevant action. Zero-argument reads execute immediately; read or write functions with inputs open a small argument sheet that accepts whitespace-separated values with quoted strings. Local write functions show a transaction preview with target address, function args, gas estimate when available, and an explicit `y` / `n` confirmation before broadcasting. Remote write functions are blocked in the TUI for now and direct users to `consol send`, which already enforces the stronger remote confirmation flow.
