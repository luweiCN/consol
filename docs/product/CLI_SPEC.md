...
The current `dev` TUI is current-contract-first instead of source-tree-first. `consol dev` scans Solidity sources under `src`, `contracts`, `test`, `script`, plus root-level single-file demos, then binds the workspace to the selected file/contract. A single discovered contract is selected automatically; multiple discovered contracts are chosen through a centered fuzzy contract picker opened with `f`. The top workspace tabs are `Dev`, `Transactions`, `Events`, `Diagnostics`, and `Settings`; the `Dev` workspace combines the active contract actions, State Watch, and Activity. `State` reuses the same zero-argument reader snapshot used by `consol state`; `Events` reuses the ABI decoded event snapshot used by `consol logs`; the contract surface reads ABI items from the built artifact and shows constructor/read/write/payable actions with explicit next-step guidance. Missing build/deployment state is reported inside the panels instead of aborting the cockpit.
...
When the selected dev target needs an artifact, startup scopes the bootstrap build to that target's source file and lets Foundry compile its transitive imports. Unrelated sources, tests, and scripts do not block the TUI from opening. An explicit project build remains the full-project health check.
...
The contract surface is the primary run surface. On wide terminals it splits into the ABI function list, a persistent State Watch, and a persistent Activity panel. Short and narrow terminals collapse those sections into a vertical/compact layout without requiring users to leave the current contract flow. The terminal UI follows a lazygit/lazydocker-style cockpit model: a compact context/status strip at the top of the workspace, a single focused runnable ABI list in the center, selected-row emphasis, selected-function details only for the active row, and a bottom keybar for the current command flow. Focus is separate from the top workspace tab: `Tab` and `Shift-Tab` cycle pane focus forward/backward inside the current workspace, while `[` and `]` switch the top workspace tab. Mouse events attach to controls already represented in the terminal—panes, tabs, status selectors, actionable headings and shortcut hints, fields, selector options, and list/action rows. Keyboard-only actions remain key hints instead of being duplicated as bracketed text buttons, and every mouse path retains a keyboard path. Function execution and Activity scrolling remain pane-scoped. Workspace-switching keys are ignored for a short guard window after mouse wheel input so terminal/tmux mixed events cannot move the user out of the current workspace. When a source file contains multiple contract/interface/library declarations, the selected target still drives the whole workspace, while the contract panel shows the sibling declarations from the same file and marks the active one; mouse clicks or `←/→` switch adjacent declarations, while `f` opens the full source-target picker. The contract function list always exposes the full deployed ABI instead of maintaining a separate visibility filter. State Watch auto-reads no-argument `view` / `pure` functions and public getters after deployment, renders decoded readable values plus raw ABI data in a compact `name / type / value` table, refreshes immediately after ConSol-owned writes, and uses 5-second polling or manual `r` refresh for external writes that happen outside the current TUI. It is not event-subscription driven yet. The State Watch view is a visualization of the same `state` section returned by `consol activity <target>` and the same reader model used by `consol state <target>`.

The selected source contract, ABI counts, active deployment instance, runnable function list, State Watch, and function execution target form one source-qualified identity. The deployment-instance picker is scoped to that identity; changing source contract must never leave another contract's instance or functions active. ABI metrics render complete labels, give non-zero custom errors danger color in addition to the warning icon, and mute zero counts. Function-kind icons label the `Read`, `Write`, and `Accepts ETH` groups instead of repeating on every item. Each item keeps its complete signature through terminal-cell-aware wrapping, while only the selected item expands named arguments and return types. A selected payable item also shows the optional ETH / wei value separately from Solidity arguments. `Enter` executes the selected function; `i` or a mouse right-click opens Function tools without executing it. Function tools can inspect the complete ABI-facing metadata and copy the overload-safe signature, 4-byte selector, single-function ABI JSON array, or ABI function declaration. In Function details, `c` copies the ABI JSON directly and `y` copies a sectioned plain-text record with stable LF line endings.

The contract panel's bottom keybar exposes `d` as `deploy new instance`; the same action is not repeated as an inline text control inside the deployment context block. The keybar uses full, compact, and keys-only labels according to the panel's terminal-cell width so the entire hint never disappears at the two-column layout boundary.

The lower-right Dev pane is labeled `Activity log` / `操作记录`, not `Transactions`: it is immediate session feedback for builds, chain lifecycle, reads, writes, refreshes, and RPC failures. The top-level `Transactions` workspace remains the durable transaction, receipt, gas, and trace surface.

The Activity layer records TUI actions, read call results, low-frequency live refresh changes, and recent local transaction history. Durable contract data comes from `consol activity <target>`, which combines deployment cache state, the `consol state` reader snapshot, the `consol logs` decoded event snapshot, and the same `.consol/transactions.json` records exposed by `consol tx list`. The compact Activity panel inside `Contract` shows that activity model plus short-lived TUI session messages such as read results, manual refresh notices, and auto-refresh notices. The compact activity view uses `tx`, `event`, and `session` prefixes and count headers so repeated reads/refreshes remain visible instead of feeling swallowed. It renders oldest-to-newest from top to bottom, follows the latest entries when new logs arrive, wraps long log entries within the panel width, and supports PageUp/PageDown plus mouse wheel scrolling back to older rendered lines with a right-side scrollbar and inline range indicator. Scroll events are pane-scoped: mouse wheel only scrolls Activity when Activity is already focused and the pointer is over that Activity pane; it does not select panes, move focus, or switch workspaces. ConSol enables only normal and SGR mouse reporting, not all-motion mouse reporting, to keep wheel input responsive under tmux. Session and transaction entries include local `HH:MM:SS` time; decoded chain events keep block/tx context and use `--:--:--` when no local timestamp is available. Pressing `t` in the Activity panel traces the latest recorded transaction hash on the active matching network and renders receipt metadata plus the first trace lines inline. The auto-refresh tick compares deployment/state/event summaries for the selected target and appends a specific activity event such as `state/activity updated` or `activity updated` when live data changes.

`consol dev` also writes session diagnostics to `~/.config/consol/logs/consol-dev.log` by default. The log includes session start/end, explicit quit reasons, terminal draw/event-loop errors, resize events, and throttled Activity scroll summaries. The panic handler restores the terminal from raw/alternate-screen mode, appends crash location, redacted CLI args, cwd, and runtime stack context when available, then prints the log path after a crash. RPC URLs and private-key-like arguments are redacted in diagnostic logs.

The first action inside `dev` is the action sheet layer: press `b` to build and refresh ABI/functions, press `d` to deploy the open target, or select the constructor row in `Contract` and press `Enter`. Constructor inputs open the same ABI argument sheet and then a deployment preview. When the `Contract` workspace is active, Up/Down move the selected ABI item, `Enter` opens or runs it, `i` opens Function tools, `←/→` switches sibling declarations in the current source file, and `c` opens the current contract's deployment-instance picker. Constructors are shown as a deploy category. `view` / `pure` functions are `read`; `nonpayable` functions are `write`; `payable` functions are labeled `PAYABLE` under `Accepts ETH`, show an optional ETH / wei value in selected-row details, and prompt for `value` before function args. The argument sheet shows input syntax, not semantic sample data: top-level arguments are separated with spaces, strings only need quotes when they contain spaces, integer values can be typed as normal decimal or `0x` hex without 256-bit zero-padding, arrays use cast-style `[v1,v2]`, and structs/tuples use `(v1,v2)` in ABI field order. Arrays and tuples are kept as a single token, so users should not add spaces after commas unless the space is inside a quoted string value. Read/write/payable function args are checked with `cast calldata` before call/send preview; constructor args are checked with `cast abi-encode constructor(...)` before deployment preview. Invalid count, quoting, numeric, array, or tuple input keeps the sheet open and shows the cast error plus the generic ABI syntax rules. If a non-constructor function is selected before the contract has an active deployment, ConSol opens the deployment args/preview flow first and explains that the function can run after deployment. Function/constructor input text is cached per `target + action + signature` for the current TUI session, so rerunning the same function restores the previous args while other functions start with their own cached args or a blank input. Zero-argument reads execute immediately after deployment. Local write functions show a transaction preview with network, chain id, account, signer, target address, value, function args, gas estimate when available, nonce/gas price when available, calldata prefix/hash, and an `Enter` confirmation before broadcasting. After deploy/write, the TUI returns to `Contract` and refreshes state so the next read/write step is visible. Remote deploy/write actions reuse the same write policy model as the CLI: `read-only` is blocked, `confirm` requires typing `yes` in the TUI, and `typed-confirm` requires typing the active network name. The TUI rechecks network, chain id, write policy, account, target address, and signer before broadcasting so a stale preview cannot silently send on a changed context.
...
The TUI text layer uses typed locale catalogs under `packages/i18n/src/locales` and the `createTranslator` helper. `[ui] language` in the ConSol config is the primary selector. Locale environment variables are fallback selectors only when the config language is absent or set to `system`. The dev workspace copy, modal labels, status messages, key hints, and help text are represented in both English and Chinese. New TUI-visible strings must add both English and Chinese locale entries in the same change, and unused keys should be removed when a surface is deleted. Semantic Nerd Font glyphs distinguish context, actions, state, and severity, but they always accompany translated text and never become the only source of meaning. Terminal cell-width calculation treats CJK text and private-use Nerd Font glyphs consistently so layout logic has one width model.
...
Gas-producing JSON payloads include a structured signal alongside scalar fields. The current signal shape is:
