...
The first feed layer records TUI actions and low-frequency live refresh changes. The `Feed` panel captures build, call, send, deploy, contract switch, network/account switch, copy, manual refresh, and auto-refresh events. The auto-refresh tick compares deployment/state/event summaries for the selected target and appends a feed event when live data changes.
